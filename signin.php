...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($user_name) || empty($password)) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT user_id, user_name, password, first_name, last_name, email, total_score, user_type, email_verified FROM users WHERE user_name = :user_name");
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                if ($user['user_type'] === 'author' && empty($user['email_verified'])) {
                    // Author is NOT yet admin-verified — send OTP as usual
                    ...
                    // Reader (never needs OTP) OR author marked verified by admin (skip OTP)
                    $_SESSION['user_id'] = $user['user_id']; 
                    $_SESSION['user_name'] = $user['user_name'];
                    ...
                    if ($user['user_type'] === 'author') {
                        header('Refresh: 1; URL=author/index.php');
                    } else {
