...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header('Location: manage_stories.php?status=error&message=No part specified.');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($content) && !empty($upload_date) && !empty($prediction_deadline)) {
        try {
            ...
            header('Location: manage_stories.php?status=success&message=Story part updated successfully.');
            exit;
...
try {
    ...
    if (!$part) {
        header('Location: manage_stories.php?status=error&message=Story part not found.');
        exit;
    }

} catch (PDOException $e) {
    error_log("Fetch Error: " . $e->getMessage());
    header('Location: manage_stories.php?status=error&message=Failed to retrieve story part data.');
    exit;
}
...
<body>
    <div class="container">
        ...
        <?php endif; ?>
        <form action="edit-story-part.php?id=<?php echo htmlspecialchars($part['part_id']); ?>" method="POST">
