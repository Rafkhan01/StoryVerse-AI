...
$stmt = $pdo->prepare("
    SELECT
        ...
        s.status, s.created_at, s.like_count, s.view_count,
...
/* Status left stripe */
.story-card.status-pending  { border-left: 3px solid var(--warning); }
.story-card.status-approved { border-left: 3px solid var(--success); }
.story-card.status-rejected { border-left: 3px solid var(--danger);  }
...
.saction-btn.delete{color:var(--danger)}
.saction-btn.delete:hover{background:#FEF2F2;border-color:#FCA5A5}
