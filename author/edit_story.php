...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header('Location: manage_story.php?status=error&message=No story specified.');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($title) && !empty($created_by) && !empty($category) && !empty($cover_image_url) && !empty($current_part_no) && !empty($total_parts)) {
        try {
            ...
            header('Location: manage_story.php?status=success&message=Story updated successfully.');
            exit;
...
try {
    ...
    if (!$story) {
        header('Location: manage_story.php?status=error&message=Story not found.');
        exit;
    }

} catch (PDOException $e) {
    error_log("Fetch Error: " . $e->getMessage());
    header('Location: manage_story.php?status=error&message=Failed to retrieve story data.');
    exit;
}
...
<body>
    <div class="container">
        ...
        <?php endif; ?>
        <form action="edit_story.php?id=<?php echo htmlspecialchars($story['story_id']); ?>" method="POST">
