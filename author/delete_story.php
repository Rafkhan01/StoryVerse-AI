...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header('Location: manage_story.php?status=error&message=No story specified for deletion.');
    exit;
}
...
try {
    ...
    $pdo->commit();
    header('Location: manage_story.php?status=success&message=Story and its parts deleted successfully.');
    exit;

} catch (PDOException $e) {
    $pdo->rollBack();
    error_log("Delete Error: " . $e->getMessage());
    header('Location: manage_story.php?status=error&message=Failed to delete story: ' . $e->getMessage());
    exit;
}
$pdo = null;
?>
