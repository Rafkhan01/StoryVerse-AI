...
switch ($action) {
    ...
    case 'get_session':
        ...
        try {
            ...
            $streak      = $progress ? (int)$progress['streak'] : 0;
    ...
    case 'submit_answer':
        ...
        // Progression rules:
        //   Advance: 3 consecutive correct answers at current WPM → go up one tier
        //   Drop:    2 consecutive wrong answers at current WPM → go down one tier
        //   Correct resets loss_streak; Wrong resets win streak
        $WINS_TO_ADVANCE  = 3;
        $LOSSES_TO_DROP   = 2;
        ...
        try {
            ...
            if ($is_correct) {
                $total_correct++;
                $new_win_streak++;
                $new_loss_streak = 0; // reset loss streak on correct
                if ($new_win_streak >= $WINS_TO_ADVANCE && $tier_index < count($WPM_TIERS) - 1) {
                    $new_wpm         = $WPM_TIERS[$tier_index + 1];
                    $new_win_streak  = 0; // reset after advancing
                    $level_changed   = true;
                    $direction       = 'up';
                }
            } else {
                $new_loss_streak++;
                $new_win_streak = 0; // reset win streak on wrong
                if ($new_loss_streak >= $LOSSES_TO_DROP && $tier_index > 0) {
                    $new_wpm         = $WPM_TIERS[$tier_index - 1];
                    $new_loss_streak = 0; // reset after dropping
                    $level_changed   = true;
                    $direction       = 'down';
                }
            }
            ...
            if ($progress) {
                if ($db_story_id === null) {
                    ...
                    $stmt->execute([$new_wpm, $new_win_streak, $new_loss_streak, $new_best,
                                    $total_correct, $total_attempts, $user_id]);
                } else {
                    $stmt = $pdo->prepare("
                        UPDATE arena_progress
                        SET current_wpm=?, streak=?, loss_streak=?, best_wpm=?,
                ...
                $stmt = $pdo->prepare("
                    ...
                    (user_id, story_id, part_number, current_wpm, streak, loss_streak,
                     best_wpm, total_correct, total_attempts)
                    VALUES (?,?,?,?,?,?,?,?,?)
                ");
                $stmt->execute([$user_id, $db_story_id, $db_part_number,
                                $new_wpm, $new_win_streak, $new_loss_streak,
                                $new_best, $total_correct, $total_attempts]);
            }
    ...
    case 'get_leaderboard':
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                ORDER BY ap.best_wpm DESC, accuracy DESC
                LIMIT 10
            ");
            $stmt->execute([$story_id, $part_number]);
